 ...
 * tests (0..1).
 */
export function rollCometReward(cps: number, random: () => number = Math.random): CometReward {
  if (random() < 0.5) {
    return { kind: 'frenzy', mult: COMET_FRENZY_MULT, durationMs: COMET_FRENZY_DURATION_MS };
  }
  return {
    kind: 'windfall',
    amount: Math.max(COMET_WINDFALL_MIN, cps * COMET_WINDFALL_SECONDS),
  };
}
